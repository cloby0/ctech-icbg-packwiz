...
ServerEvents.recipes(event => {
    ...
    // Drop SG's own alloy-ingot crafts (bench recipes + nugget/block conversions + Metal Alloyer
    // compounding). Exact silentgear: IDs only, verified against silent-gear-1.20.1-3.6.7.jar - NOT
    // output-matching. These ingots are setIgnored (canonical), so GT's own autogenerated EBF/
    // vacuum-freezer recipes also output silentgear:x_ingot; a broad output-match removal deletes
    // those too, leaving no path to the ingot at all (or a hot ingot with no way to cool it).
    ;[
        'blaze_gold_ingot', 'blaze_gold_ingot_from_block', 'blaze_gold_ingot_from_nugget', 'compounding/metal/blaze_gold_ingot',
        'azure_electrum_ingot', 'azure_electrum_ingot_from_block', 'azure_electrum_ingot_from_nugget',
        'crimson_steel_ingot', 'crimson_steel_ingot_from_block', 'crimson_steel_ingot_from_nugget', 'compounding/metal/crimson_steel_ingot',
        'tyrian_steel_ingot_from_block', 'tyrian_steel_ingot_from_nugget', 'compounding/metal/tyrian_steel_ingot'
    ].forEach(recipe => {
        event.remove({ id: SG + ':' + recipe })
    })
    ...
    // ---- Bucket F: star-themed machine blocks ----

    // Starlight charger draws on starlight, so it's built at the enchanting apparatus (Initiate tier).
    event.remove({ id: SG + ':starlight_charger' })
    addEnchantingRecipe(event, {
        reagent: 'minecraft:quartz_block',
        output: SG + ':starlight_charger',
        sourceCost: 2 * Source.INITIATE,
        pedestalItems: ['gtceu:blaze_gold_plate', SG + ':starmetal_dust', '#forge:glass/colorless', 'minecraft:polished_blackstone']
    })
