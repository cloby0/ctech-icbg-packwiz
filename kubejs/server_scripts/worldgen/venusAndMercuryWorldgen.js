GTCEuServerEvents.oreVeins(event => {
    ...
    // solarmetal is LuV-tier locked to Mercury — Scheelite companion signals tungsten/manganese byproduct chain
    event.add("kubejs:solarmetal_vein_mc", vein => {
        vein.weight(12)
        vein.clusterSize(18)
        vein.density(0.45)
        vein.discardChanceOnAirExposure(0)
        ...
        vein.layeredVeinGenerator(generator => generator
            .buildLayerPattern(pattern => pattern
                .layer(l => l.weight(3).mat(GTMaterials.get("solarmetal")).size(2, 4))
                .layer(l => l.weight(3).mat(GTMaterials.Scheelite).size(2, 4))
                .layer(l => l.weight(2).mat(GTMaterials.Pyrolusite).size(1, 3))
                .layer(l => l.weight(1).mat(GTMaterials.Bastnasite).size(1, 2))
            )
        )
        vein.surfaceIndicatorGenerator(indicator => indicator
